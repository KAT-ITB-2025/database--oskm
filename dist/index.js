import {pgEnum,pgTable,timestamp,text,boolean,integer,serial,primaryKey,real,pgMaterializedView,time,bigserial}from'drizzle-orm/pg-core';import {init,createId}from'@paralleldrive/cuid2';import {sql,relations}from'drizzle-orm';var o=init({length:8}),a=()=>new Date;var Ne=pgEnum("accounts_role_enum",["admin","mamet","mentor","user","guest","hr"]),E=pgTable("accounts",{id:text("id").primaryKey().$defaultFn(()=>o()),nim:text("nim").notNull().unique(),password:text("password").notNull(),role:Ne("role").default("user").notNull(),lastLoggedIn:timestamp("last_logged_in"),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)});var n=pgTable("media",{id:text("id").primaryKey().$defaultFn(()=>o()),creatorId:text("creator_id").references(()=>t.id),name:text("name").notNull(),bucket:text("bucket").notNull(),type:text("type").notNull(),url:text("url").notNull(),updatedAt:timestamp("updated_at").$onUpdate(a),createdAt:timestamp("created_at").defaultNow()});var t=pgTable("users",{id:text("id").references(()=>E.id).primaryKey(),nim:text("nim").notNull().unique(),email:text("email").unique(),emailVerified:boolean("email_verified").notNull().default(false),fullName:text("full_name"),fakultas:text("fakultas"),keluarga:text("keluarga"),bata:text("bata"),rumpun:text("rumpun"),fotoMediaId:text("foto_media_id").references(()=>n.id),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)}),$=pgTable("email_verification_otps",{userId:text("user_id").notNull().references(()=>t.id,{onDelete:"cascade"}),otp:text("otp").notNull(),expiresAt:timestamp("expires_at",{mode:"date",withTimezone:true}).notNull(),createdAt:timestamp("created_at").defaultNow()});var W=pgTable("endpoint_analytics",{id:serial("id").primaryKey(),userId:text("user_id").references(()=>t.id,{onDelete:"set null"}),endpoint:text("endpoint").notNull(),method:text("method").notNull(),statusCode:integer("status_code").notNull(),responseTimeMs:integer("response_time_ms"),urlQuery:text("url_query"),requestBody:text("request_body"),errorMessage:text("error_message"),createdAt:timestamp("created_at").defaultNow()});var J=pgTable("verification_token",{identifier:text("identifier").notNull().references(()=>t.email),token:text("token").notNull(),expiredAt:timestamp("expired_at",{mode:"date",withTimezone:true})},e=>[primaryKey({columns:[e.identifier,e.token]})]);var ve=pgEnum("class_enum",["sesi_1","sesi_2"]),D=pgTable("classes",{id:text("id").primaryKey().$defaultFn(()=>o()),className:text("class_name").notNull(),room:text("room").notNull(),totalQuota:integer("total_quota").notNull(),mentorName:text("mentor_name").notNull(),classType:ve("class_type").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)}),T=pgTable("class_registrations",{id:text("id").primaryKey().$defaultFn(()=>o()),userId:text("user_id").notNull().references(()=>t.id),classId:text("class_id").notNull().references(()=>D.id),registeredAt:timestamp("registered_at").defaultNow(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)});var _=pgTable("profil_kats",{id:text("id").primaryKey().$defaultFn(()=>o()),profilNumber:integer("profil_number").notNull().unique(),stageWeight:real("stage_weight").notNull().default(0),quizWeight:real("quiz_weight").notNull().default(0),assignmentWeight:real("assignment_weight").notNull().default(0),attendanceWeight:real("attendance_weight").notNull().default(0),title:text("title").notNull(),description:text("description")}),h=pgTable("assignments_profil",{id:text("id").primaryKey().$defaultFn(()=>o()),profilKATId:text("profil_kat_id").notNull().references(()=>_.id),title:text("title").notNull(),assignmentMediaId:text("assignment_media_id").references(()=>n.id),description:text("description"),dueDate:timestamp("due_date",{mode:"date",withTimezone:true}).notNull(),isOpen:boolean("is_open").default(false).notNull(),startDate:timestamp("start_date",{mode:"date",withTimezone:true}).notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)}),g=pgTable("submissions_profil",{id:text("id").primaryKey().$defaultFn(()=>o()),assignmentId:text("assignment_id").notNull().references(()=>h.id),userId:text("user_id").notNull().references(()=>t.id),submissionMediaId:text("submission_media_id").notNull().references(()=>n.id),score:integer("score").default(0).notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)});var Ie=pgEnum("status",["completed","not_completed"]),p=pgTable("stages",{id:text("id").primaryKey().$defaultFn(()=>o()),profilId:text("profil_id").notNull().references(()=>_.id),stageNumber:integer("stage_number").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)}),O=pgTable("user_stage_progress",{id:text("id").primaryKey().$defaultFn(()=>o()),userId:text("user_id").notNull().references(()=>t.id),stageId:text("stage_id").notNull().references(()=>p.id),status:Ie("status").notNull(),completedAt:timestamp("completed_at"),quizScore:integer("quiz_score"),updatedAt:timestamp("updated_at").$onUpdate(a)});var xe=pgEnum("question_type_enum",["multiple_choice","short_answer"]),w=pgTable("questions",{id:text("id").primaryKey().$defaultFn(()=>o()),stageId:text("stage_id").notNull().references(()=>p.id),questionText:text("question_text").notNull(),questionType:xe("question_type").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)}),U=pgTable("question_answer_options",{id:text("id").primaryKey().$defaultFn(()=>o()),questionId:text("question_id").notNull().references(()=>w.id),answerText:text("answer_text").notNull(),isCorrect:boolean("is_correct").default(false).notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)});var Bt=pgMaterializedView("user_ranking_view",{userId:text("user_id").primaryKey(),nim:text("nim"),fullName:text("full_name"),fakultas:text("fakultas"),keluarga:text("keluarga"),bata:text("bata"),rumpun:text("rumpun"),fotoMediaId:text("foto_media_id"),profil1QuizWeight:real("profil1_quiz_weight").notNull().default(0),profil1QuizScore:integer("profil1_quiz_score").notNull().default(0),profil1AssignmentWeight:real("profil1_assignment_weight").notNull().default(0),profil1AvgAssignmentScore:real("profil1_avg_assignment_score").notNull().default(0),profil1AttendanceWeight:real("profil1_attendance_weight").notNull().default(0),profil1AvgAttendanceScore:real("profil1_avg_attendance_score").notNull().default(0),profil1TotalScore:real("profil1_total_score").notNull().default(0),profil2QuizWeight:real("profil2_quiz_weight").notNull().default(0),profil2QuizScore:integer("profil2_quiz_score").notNull().default(0),profil2AssignmentWeight:real("profil2_assignment_weight").notNull().default(0),profil2AvgAssignmentScore:real("profil2_avg_assignment_score").notNull().default(0),profil2AttendanceWeight:real("profil2_attendance_weight").notNull().default(0),profil2AvgAttendanceScore:real("profil2_avg_attendance_score").notNull().default(0),profil2TotalScore:real("profil2_total_score").notNull().default(0),profil3QuizWeight:real("profil3_quiz_weight").notNull().default(0),profil3QuizScore:integer("profil3_quiz_score").notNull().default(0),profil3AssignmentWeight:real("profil3_assignment_weight").notNull().default(0),profil3AvgAssignmentScore:real("profil3_avg_assignment_score").notNull().default(0),profil3AttendanceWeight:real("profil3_attendance_weight").notNull().default(0),profil3AvgAttendanceScore:real("profil3_avg_attendance_score").notNull().default(0),profil3TotalScore:real("profil3_total_score").notNull().default(0),profil4QuizWeight:real("profil4_quiz_weight").notNull().default(0),profil4QuizScore:integer("profil4_quiz_score").notNull().default(0),profil4AssignmentWeight:real("profil4_assignment_weight").notNull().default(0),profil4AvgAssignmentScore:real("profil4_avg_assignment_score").notNull().default(0),profil4AttendanceWeight:real("profil4_attendance_weight").notNull().default(0),profil4AvgAttendanceScore:real("profil4_avg_attendance_score").notNull().default(0),profil4TotalScore:real("profil4_total_score").notNull().default(0),profil5QuizWeight:real("profil5_quiz_weight").notNull().default(0),profil5QuizScore:integer("profil5_quiz_score").notNull().default(0),profil5AssignmentWeight:real("profil5_assignment_weight").notNull().default(0),profil5AvgAssignmentScore:real("profil5_avg_assignment_score").notNull().default(0),profil5AttendanceWeight:real("profil5_attendance_weight").notNull().default(0),profil5AvgAttendanceScore:real("profil5_avg_attendance_score").notNull().default(0),profil5TotalScore:real("profil5_total_score").notNull().default(0),totalScore:real("total_score").notNull().default(0),totalWeightedScore:real("total_weighted_score").notNull().default(0),tiebreakerScore:real("tiebreaker_score").notNull().default(0),lastActivityAt:timestamp("last_activity_at"),attendanceTotal:integer("attendance_total").notNull().default(0),ranking:integer("ranking").notNull()}).as(sql`
  ...
  FROM user_scores
  ORDER BY ranking
`);async function Yt(e){await e.execute(sql`REFRESH MATERIALIZED VIEW CONCURRENTLY user_ranking_view`);}var Zt=sql`
...
`;var Me=pgEnum("attendance_status",["hadir","tidak_hadir"]),Re=pgEnum("attendance_type",["opening","closing"]),c=pgTable("attendances",{id:text("id").primaryKey().$defaultFn(()=>o()),attendanceType:Re("attendance_type").notNull(),dayNumber:integer("day_number").notNull(),startTime:timestamp("start_time",{mode:"date",withTimezone:true}).notNull(),durationMinutes:integer("duration_minutes").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)}),b=pgTable("profil_kat_attendance",{id:text("id").primaryKey().$defaultFn(()=>o()),profilKATId:text("profil_kat_id").notNull().references(()=>_.id),attendanceId:text("attendance_id").notNull().references(()=>c.id),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)}),L=pgTable("user_attendance",{id:text("id").primaryKey().$defaultFn(()=>o()),scheduleId:text("schedule_id").notNull().references(()=>c.id),userId:text("user_id").notNull().references(()=>t.id),status:Me("status").default("tidak_hadir").notNull(),updatedAt:timestamp("updated_at").$onUpdate(a)});var B=pgTable("handbook",{mediaId:text("media_id").primaryKey().references(()=>n.id),title:text("title")});var Ti=pgTable("activities",{id:text("id").primaryKey().$defaultFn(()=>o()),day:integer("day").notNull(),title:text("title").notNull(),description:text("description"),startTime:time("start_time").notNull(),endTime:time("end_time").notNull(),location:text("location"),lat:integer("lat"),lng:integer("lng"),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)});var f=pgTable("user_matches",{id:text("id").primaryKey().$defaultFn(createId),topic:text("topic").notNull(),isActive:boolean("is_active").notNull().default(true),isAnonymous:boolean("is_anonymous").notNull().default(true),isRevealed:boolean("is_revealed").notNull().default(false),firstUserId:text("first_user_id").notNull().references(()=>t.id,{onDelete:"cascade"}),secondUserId:text("second_user_id").notNull().references(()=>t.id,{onDelete:"cascade"}),lastMessage:text("last_message"),createdAt:timestamp("created_at").defaultNow()}),I=pgTable("messages",{id:bigserial("id",{mode:"number"}).primaryKey(),userMatchId:text("user_match_id").notNull().references(()=>f.id,{onDelete:"cascade"}),senderId:text("sender_id").notNull().references(()=>t.id,{onDelete:"cascade"}),content:text("content").notNull(),createdAt:timestamp("created_at",{mode:"date",withTimezone:true})});var Ki=relations(E,({one:e})=>({user:e(t,{fields:[E.id],references:[t.id]})})),$i=relations(t,({one:e,many:r})=>({account:e(E,{fields:[t.id],references:[E.id]}),fotoMedia:e(n,{fields:[t.fotoMediaId],references:[n.id]}),stageProgress:r(O),attendance:r(L),submissions:r(g),createdMedia:r(n),analytics:r(W),classRegistrations:r(T),userMatchesAsFirstUser:r(f,{relationName:"first_user"}),userMatchesAsSecondUser:r(f,{relationName:"second_user"}),messages:r(I,{relationName:"sender"})})),Ji=relations($,({one:e})=>({user:e(t,{fields:[$.userId],references:[t.id]})})),Gi=relations(J,({one:e})=>({user:e(t,{fields:[J.identifier],references:[t.email]})})),Qi=relations(n,({one:e,many:r})=>({creator:e(t,{fields:[n.creatorId],references:[t.id]}),handbook:e(B,{fields:[n.id],references:[B.mediaId]}),submissions:r(g)})),Vi=relations(c,({many:e})=>({userAttendance:e(L),profilKATAttendances:e(b)})),Bi=relations(L,({one:e})=>({attendance:e(c,{fields:[L.scheduleId],references:[c.id]}),user:e(t,{fields:[L.userId],references:[t.id]})})),Yi=relations(b,({one:e})=>({profilKAT:e(_,{fields:[b.profilKATId],references:[_.id]}),attendance:e(c,{fields:[b.attendanceId],references:[c.id]})})),Zi=relations(p,({many:e,one:r})=>({userProgress:e(O),questions:e(w),profil:r(_,{fields:[p.profilId],references:[_.id]})})),ji=relations(O,({one:e})=>({user:e(t,{fields:[O.userId],references:[t.id]}),stage:e(p,{fields:[O.stageId],references:[p.id]})})),er=relations(w,({one:e,many:r})=>({stage:e(p,{fields:[w.stageId],references:[p.id]}),answerOptions:r(U)})),tr=relations(U,({one:e})=>({question:e(w,{fields:[U.questionId],references:[w.id]})})),ir=relations(_,({many:e,one:r})=>({assignments:e(h),attendances:e(b),stage:r(p,{fields:[_.id],references:[p.profilId]})})),rr=relations(h,({many:e,one:r})=>({submissions:e(g),profil:r(_,{fields:[h.profilKATId],references:[_.id]})})),ar=relations(g,({one:e})=>({assignment:e(h,{fields:[g.assignmentId],references:[h.id]}),user:e(t,{fields:[g.userId],references:[t.id]}),media:e(n,{fields:[g.submissionMediaId],references:[n.id]})})),or=relations(D,({one:e,many:r})=>({registrations:r(T)})),sr=relations(T,({one:e})=>({user:e(t,{fields:[T.userId],references:[t.id]}),class:e(D,{fields:[T.classId],references:[D.id]})})),nr=relations(W,({one:e})=>({user:e(t,{fields:[W.userId],references:[t.id]})})),_r=relations(f,({many:e,one:r})=>({firstUser:r(t,{fields:[f.firstUserId],references:[t.id],relationName:"first_user"}),secondUser:r(t,{fields:[f.secondUserId],references:[t.id],relationName:"second_user"}),messages:e(I)})),lr=relations(I,({one:e})=>({senderId:e(t,{fields:[I.senderId],references:[t.id],relationName:"sender"}),userMatch:e(f,{fields:[I.userMatchId],references:[f.id]})}));export{E as accounts,Ki as accountsRelation,Ne as accountsRoleEnum,Ti as activities,h as assignmentsProfil,rr as assignmentsProfilRelation,Me as attendanceStatusEnum,Re as attendanceTypeEnum,c as attendances,Vi as attendancesRelation,ve as classEnum,T as classRegistrations,sr as classRegistrationsRelation,D as classes,or as classesRelation,Zt as createUserRankingViewIndexes,$ as emailVerificationOtps,Ji as emailVerificationOtpsRelations,W as endpointAnalytics,nr as endpointAnalyticsRelation,B as handbook,n as media,Qi as mediaRelation,I as messages,lr as messagesRelations,b as profilKATAttendances,Yi as profilKATAttendancesRelation,_ as profilKATs,ir as profilKATsRelation,U as questionAnswerOptions,tr as questionAnswerOptionsRelation,xe as questionTypeEnum,w as questions,er as questionsRelation,Yt as refreshUserRankingView,Ie as stageStatusEnum,p as stages,Zi as stagesRelation,g as submissionsProfil,ar as submissionsProfilRelation,L as userAttendance,Bi as userAttendanceRelation,f as userMatches,_r as userMatchesRelations,Bt as userRankingView,O as userStageProgress,ji as userStageProgressRelation,t as users,$i as usersRelation,J as verificationToken,Gi as verificationTokenRelations};