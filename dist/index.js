import {pgEnum,pgTable,timestamp,text,boolean,integer,serial,primaryKey,real,pgMaterializedView,time,bigserial}from'drizzle-orm/pg-core';import {init,createId}from'@paralleldrive/cuid2';import {sql,relations}from'drizzle-orm';var a=init({length:8}),r=()=>new Date;var Oe=pgEnum("accounts_role_enum",["admin","mamet","mentor","user","guest","hr"]),h=pgTable("accounts",{id:text("id").primaryKey().$defaultFn(()=>a()),nim:text("nim").notNull().unique(),password:text("password").notNull(),role:Oe("role").default("user").notNull(),lastLoggedIn:timestamp("last_logged_in"),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)});var n=pgTable("media",{id:text("id").primaryKey().$defaultFn(()=>a()),creatorId:text("creator_id").references(()=>t.id),name:text("name").notNull(),bucket:text("bucket").notNull(),type:text("type").notNull(),url:text("url").notNull(),updatedAt:timestamp("updated_at").$onUpdate(r),createdAt:timestamp("created_at").defaultNow()});var t=pgTable("users",{id:text("id").references(()=>h.id).primaryKey(),nim:text("nim").notNull().unique(),email:text("email").unique(),emailVerified:boolean("email_verified").notNull().default(false),fullName:text("full_name").notNull(),fakultas:text("fakultas").notNull(),keluarga:text("keluarga").notNull(),bata:text("bata").notNull(),rumpun:text("rumpun"),fotoMediaId:text("foto_media_id").references(()=>n.id),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),Q=pgTable("email_verification_otps",{userId:text("user_id").notNull().references(()=>t.id,{onDelete:"cascade"}),otp:text("otp").notNull(),expiresAt:timestamp("expires_at",{mode:"date",withTimezone:true}).notNull(),createdAt:timestamp("created_at").defaultNow()});var R=pgTable("endpoint_analytics",{id:serial("id").primaryKey(),userId:text("user_id").references(()=>t.id,{onDelete:"set null"}),endpoint:text("endpoint").notNull(),method:text("method").notNull(),statusCode:integer("status_code").notNull(),responseTimeMs:integer("response_time_ms"),urlQuery:text("url_query"),requestBody:text("request_body"),errorMessage:text("error_message"),createdAt:timestamp("created_at").defaultNow()});var V=pgTable("verification_token",{identifier:text("identifier").notNull().references(()=>t.email),token:text("token").notNull(),expiredAt:timestamp("expired_at",{mode:"date",withTimezone:true})},e=>[primaryKey({columns:[e.identifier,e.token]})]);var Me=pgEnum("class_enum",["sesi_1","sesi_2"]),k=pgTable("classes",{id:text("id").primaryKey().$defaultFn(()=>a()),className:text("class_name").notNull(),room:text("room").notNull(),totalQuota:integer("total_quota").notNull(),mentorName:text("mentor_name").notNull(),description:text("description").notNull(),classType:Me("class_type").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),T=pgTable("class_registrations",{id:text("id").primaryKey().$defaultFn(()=>a()),userId:text("user_id").notNull().references(()=>t.id),classId:text("class_id").notNull().references(()=>k.id),registeredAt:timestamp("registered_at").defaultNow(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)});var _=pgTable("profil_kats",{id:text("id").primaryKey().$defaultFn(()=>a()),profilNumber:integer("profil_number").notNull().unique(),stageWeight:real("stage_weight").notNull().default(0),quizWeight:real("quiz_weight").notNull().default(0),assignmentWeight:real("assignment_weight").notNull().default(0),attendanceWeight:real("attendance_weight").notNull().default(0),title:text("title").notNull(),description:text("description")}),w=pgTable("assignments_profil",{id:text("id").primaryKey().$defaultFn(()=>a()),profilKATId:text("profil_kat_id").notNull().references(()=>_.id),title:text("title").notNull(),assignmentMediaId:text("assignment_media_id").references(()=>n.id),description:text("description"),dueDate:timestamp("due_date",{mode:"date",withTimezone:true}).notNull(),isOpen:boolean("is_open").default(false).notNull(),startDate:timestamp("start_date",{mode:"date",withTimezone:true}).notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),d=pgTable("submissions_profil",{id:text("id").primaryKey().$defaultFn(()=>a()),assignmentId:text("assignment_id").notNull().references(()=>w.id),userId:text("user_id").notNull().references(()=>t.id),submissionMediaId:text("submission_media_id").notNull().references(()=>n.id),score:integer("score").default(0).notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)});var Ue=pgEnum("status",["completed","not_completed"]),u=pgTable("stages",{id:text("id").primaryKey().$defaultFn(()=>a()),profilId:text("profil_id").notNull().references(()=>_.id),stageNumber:integer("stage_number").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),O=pgTable("user_stage_progress",{id:text("id").primaryKey().$defaultFn(()=>a()),userId:text("user_id").notNull().references(()=>t.id),stageId:text("stage_id").notNull().references(()=>u.id),status:Ue("status").notNull(),completedAt:timestamp("completed_at"),quizScore:integer("quiz_score"),updatedAt:timestamp("updated_at").$onUpdate(r)});var Ke=pgEnum("question_type_enum",["multiple_choice","short_answer"]),A=pgTable("questions",{id:text("id").primaryKey().$defaultFn(()=>a()),stageId:text("stage_id").notNull().references(()=>u.id),questionText:text("question_text").notNull(),questionType:Ke("question_type").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),P=pgTable("question_answer_options",{id:text("id").primaryKey().$defaultFn(()=>a()),questionId:text("question_id").notNull().references(()=>A.id),answerText:text("answer_text").notNull(),isCorrect:boolean("is_correct").default(false).notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)});var di=pgMaterializedView("user_ranking_view",{userId:text("user_id").primaryKey(),nim:text("nim"),fullName:text("full_name"),fakultas:text("fakultas"),keluarga:text("keluarga"),bata:text("bata"),rumpun:text("rumpun"),fotoMediaId:text("foto_media_id"),profil1QuizWeight:real("profil1_quiz_weight").notNull().default(0),profil1QuizScore:integer("profil1_quiz_score").notNull().default(0),profil1AssignmentWeight:real("profil1_assignment_weight").notNull().default(0),profil1AvgAssignmentScore:real("profil1_avg_assignment_score").notNull().default(0),profil1AttendanceWeight:real("profil1_attendance_weight").notNull().default(0),profil1AvgAttendanceScore:real("profil1_avg_attendance_score").notNull().default(0),profil1TotalScore:real("profil1_total_score").notNull().default(0),profil2QuizWeight:real("profil2_quiz_weight").notNull().default(0),profil2QuizScore:integer("profil2_quiz_score").notNull().default(0),profil2AssignmentWeight:real("profil2_assignment_weight").notNull().default(0),profil2AvgAssignmentScore:real("profil2_avg_assignment_score").notNull().default(0),profil2AttendanceWeight:real("profil2_attendance_weight").notNull().default(0),profil2AvgAttendanceScore:real("profil2_avg_attendance_score").notNull().default(0),profil2TotalScore:real("profil2_total_score").notNull().default(0),profil3QuizWeight:real("profil3_quiz_weight").notNull().default(0),profil3QuizScore:integer("profil3_quiz_score").notNull().default(0),profil3AssignmentWeight:real("profil3_assignment_weight").notNull().default(0),profil3AvgAssignmentScore:real("profil3_avg_assignment_score").notNull().default(0),profil3AttendanceWeight:real("profil3_attendance_weight").notNull().default(0),profil3AvgAttendanceScore:real("profil3_avg_attendance_score").notNull().default(0),profil3TotalScore:real("profil3_total_score").notNull().default(0),profil4QuizWeight:real("profil4_quiz_weight").notNull().default(0),profil4QuizScore:integer("profil4_quiz_score").notNull().default(0),profil4AssignmentWeight:real("profil4_assignment_weight").notNull().default(0),profil4AvgAssignmentScore:real("profil4_avg_assignment_score").notNull().default(0),profil4AttendanceWeight:real("profil4_attendance_weight").notNull().default(0),profil4AvgAttendanceScore:real("profil4_avg_attendance_score").notNull().default(0),profil4TotalScore:real("profil4_total_score").notNull().default(0),profil5QuizWeight:real("profil5_quiz_weight").notNull().default(0),profil5QuizScore:integer("profil5_quiz_score").notNull().default(0),profil5AssignmentWeight:real("profil5_assignment_weight").notNull().default(0),profil5AvgAssignmentScore:real("profil5_avg_assignment_score").notNull().default(0),profil5AttendanceWeight:real("profil5_attendance_weight").notNull().default(0),profil5AvgAttendanceScore:real("profil5_avg_attendance_score").notNull().default(0),profil5TotalScore:real("profil5_total_score").notNull().default(0),itbGuesserMaxScore:real("itb_guesser_max_score").notNull().default(0),memoryGameScore:integer("memory_game_score").notNull().default(0),totalScore:real("total_score").notNull().default(0),totalWeightedScore:real("total_weighted_score").notNull().default(0),tiebreakerScore:real("tiebreaker_score").notNull().default(0),lastActivityAt:timestamp("last_activity_at"),attendanceTotal:integer("attendance_total").notNull().default(0),ranking:integer("ranking").notNull()}).as(sql`
  ...
  FROM user_scores
  ORDER BY ranking
`);async function ci(e){await e.execute(sql`REFRESH MATERIALIZED VIEW CONCURRENTLY user_ranking_view`);}var mi=sql`
...
`;var Je=pgEnum("attendance_status",["hadir","tidak_hadir"]),Qe=pgEnum("attendance_type",["opening","closing"]),E=pgTable("attendances",{id:text("id").primaryKey().$defaultFn(()=>a()),attendanceType:Qe("attendance_type").notNull(),dayNumber:integer("day_number").notNull(),startTime:timestamp("start_time",{mode:"date",withTimezone:true}).notNull(),durationMinutes:integer("duration_minutes").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),v=pgTable("profil_kat_attendance",{id:text("id").primaryKey().$defaultFn(()=>a()),profilKATId:text("profil_kat_id").notNull().references(()=>_.id),attendanceId:text("attendance_id").notNull().references(()=>E.id),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),L=pgTable("user_attendance",{id:text("id").primaryKey().$defaultFn(()=>a()),scheduleId:text("schedule_id").notNull().references(()=>E.id),userId:text("user_id").notNull().references(()=>t.id),status:Je("status").default("tidak_hadir").notNull(),updatedAt:timestamp("updated_at").$onUpdate(r)});var Ye=pgEnum("itb_guesser_tempat",["Ganesha","Jatinangor","Cirebon"]),de=pgTable("itb_guesser_options",{id:text("id").primaryKey().$defaultFn(()=>a()),tempat:Ye("tempat").notNull(),publicUrl:text("public_url").notNull(),lat:real("lat").notNull(),lng:real("lng").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),K=pgTable("itb_guesser_submissions",{id:text("id").primaryKey().$defaultFn(()=>a()),userId:text("user_id").notNull().references(()=>t.id),score:real("score").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),ce=pgTable("memory_game_scores",{id:text("id").primaryKey().$defaultFn(()=>a()),userId:text("user_id").notNull().unique().references(()=>t.id),score:integer("score").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)});var tt=pgEnum("personality",["Direwolf","Gigantopithecus","Jeholornis","Brontothere","Saber Tooth","Mammoth","Deinonychus","T-Rex","Diplodocus","Troodon","Quetzalcoatlus","Parasaurolophus","Velociraptor","Ankylosaurus","Megaloceros","Argentavis Magnificens"]),Ee=pgTable("user_personality",{userId:text("user_id").notNull().references(()=>t.id),personality:tt("personality").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)});var te=pgTable("handbook",{mediaId:text("media_id").primaryKey().references(()=>n.id),title:text("title")});var er=pgTable("activities",{id:text("id").primaryKey().$defaultFn(()=>a()),day:integer("day").notNull(),title:text("title").notNull(),description:text("description"),startTime:time("start_time").notNull(),endTime:time("end_time").notNull(),location:text("location"),lat:real("lat"),lng:real("lng"),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)});var f=pgTable("user_matches",{id:text("id").primaryKey().$defaultFn(createId),topic:text("topic").notNull(),isActive:boolean("is_active").notNull().default(true),isAnonymous:boolean("is_anonymous").notNull().default(true),isRevealed:boolean("is_revealed").notNull().default(false),firstUserId:text("first_user_id").notNull().references(()=>t.id,{onDelete:"cascade"}),secondUserId:text("second_user_id").notNull().references(()=>t.id,{onDelete:"cascade"}),lastMessage:text("last_message"),createdAt:timestamp("created_at").notNull().defaultNow()}),x=pgTable("messages",{id:bigserial("id",{mode:"number"}).primaryKey(),userMatchId:text("user_match_id").notNull().references(()=>f.id,{onDelete:"cascade"}),senderId:text("sender_id").notNull().references(()=>t.id,{onDelete:"cascade"}),content:text("content").notNull(),createdAt:timestamp("created_at",{mode:"date",withTimezone:true}).notNull().defaultNow()});var hr=relations(h,({one:e})=>({user:e(t,{fields:[h.id],references:[t.id]})})),wr=relations(t,({one:e,many:s})=>({account:e(h,{fields:[t.id],references:[h.id]}),fotoMedia:e(n,{fields:[t.fotoMediaId],references:[n.id]}),stageProgress:s(O),attendance:s(L),submissions:s(d),createdMedia:s(n),analytics:s(R),classRegistrations:s(T),userMatchesAsFirstUser:s(f,{relationName:"first_user"}),userMatchesAsSecondUser:s(f,{relationName:"second_user"}),messages:s(x,{relationName:"sender"}),itbGuesserSubmissions:s(K),memoryGameScore:e(ce),personality:e(Ee)})),Ar=relations(Q,({one:e})=>({user:e(t,{fields:[Q.userId],references:[t.id]})})),Cr=relations(V,({one:e})=>({user:e(t,{fields:[V.identifier],references:[t.email]})})),Sr=relations(n,({one:e,many:s})=>({creator:e(t,{fields:[n.creatorId],references:[t.id]}),handbook:e(te,{fields:[n.id],references:[te.mediaId]}),submissions:s(d),itbGuesserOptions:s(de)})),qr=relations(E,({many:e})=>({userAttendance:e(L),profilKATAttendances:e(v)})),Tr=relations(L,({one:e})=>({attendance:e(E,{fields:[L.scheduleId],references:[E.id]}),user:e(t,{fields:[L.userId],references:[t.id]})})),zr=relations(v,({one:e})=>({profilKAT:e(_,{fields:[v.profilKATId],references:[_.id]}),attendance:e(E,{fields:[v.attendanceId],references:[E.id]})})),Or=relations(u,({many:e,one:s})=>({userProgress:e(O),questions:e(A),profil:s(_,{fields:[u.profilId],references:[_.id]})})),br=relations(O,({one:e})=>({user:e(t,{fields:[O.userId],references:[t.id]}),stage:e(u,{fields:[O.stageId],references:[u.id]})})),Hr=relations(A,({one:e,many:s})=>({stage:e(u,{fields:[A.stageId],references:[u.id]}),answerOptions:s(P)})),vr=relations(P,({one:e})=>({question:e(A,{fields:[P.questionId],references:[A.id]})})),Lr=relations(_,({many:e,one:s})=>({assignments:e(w),attendances:e(v),stage:s(u,{fields:[_.id],references:[u.profilId]})})),yr=relations(w,({many:e,one:s})=>({submissions:e(d),profil:s(_,{fields:[w.profilKATId],references:[_.id]})})),Ir=relations(d,({one:e})=>({assignment:e(w,{fields:[d.assignmentId],references:[w.id]}),user:e(t,{fields:[d.userId],references:[t.id]}),media:e(n,{fields:[d.submissionMediaId],references:[n.id]})})),xr=relations(k,({one:e,many:s})=>({registrations:s(T)})),Dr=relations(T,({one:e})=>({user:e(t,{fields:[T.userId],references:[t.id]}),class:e(k,{fields:[T.classId],references:[k.id]})})),kr=relations(R,({one:e})=>({user:e(t,{fields:[R.userId],references:[t.id]})})),Wr=relations(f,({many:e,one:s})=>({firstUser:s(t,{fields:[f.firstUserId],references:[t.id],relationName:"first_user"}),secondUser:s(t,{fields:[f.secondUserId],references:[t.id],relationName:"second_user"}),messages:e(x)})),Mr=relations(x,({one:e})=>({senderId:e(t,{fields:[x.senderId],references:[t.id],relationName:"sender"}),userMatch:e(f,{fields:[x.userMatchId],references:[f.id]})})),Rr=relations(K,({one:e})=>({user:e(t,{fields:[K.userId],references:[t.id]})}));export{h as accounts,hr as accountsRelation,Oe as accountsRoleEnum,er as activities,w as assignmentsProfil,yr as assignmentsProfilRelation,Je as attendanceStatusEnum,Qe as attendanceTypeEnum,E as attendances,qr as attendancesRelation,Me as classEnum,T as classRegistrations,Dr as classRegistrationsRelation,k as classes,xr as classesRelation,mi as createUserRankingViewIndexes,Q as emailVerificationOtps,Ar as emailVerificationOtpsRelations,R as endpointAnalytics,kr as endpointAnalyticsRelation,te as handbook,de as itbGuesserOptions,K as itbGuesserSubmissions,Rr as itbGuesserSubmissionsRelations,Ye as itbGuesserTempatEnum,n as media,Sr as mediaRelation,ce as memoryGameScores,x as messages,Mr as messagesRelations,tt as personalityEnum,v as profilKATAttendances,zr as profilKATAttendancesRelation,_ as profilKATs,Lr as profilKATsRelation,P as questionAnswerOptions,vr as questionAnswerOptionsRelation,Ke as questionTypeEnum,A as questions,Hr as questionsRelation,ci as refreshUserRankingView,Ue as stageStatusEnum,u as stages,Or as stagesRelation,d as submissionsProfil,Ir as submissionsProfilRelation,L as userAttendance,Tr as userAttendanceRelation,f as userMatches,Wr as userMatchesRelations,Ee as userPersonality,di as userRankingView,O as userStageProgress,br as userStageProgressRelation,t as users,wr as usersRelation,V as verificationToken,Cr as verificationTokenRelations};