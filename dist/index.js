...
`;var Ke=pgEnum("attendance_status",["hadir","tidak_hadir"]),$e=pgEnum("attendance_type",["opening","closing"]),E=pgTable("attendances",{id:text("id").primaryKey().$defaultFn(()=>s()),attendanceType:$e("attendance_type").notNull(),dayNumber:integer("day_number").notNull(),startTime:timestamp("start_time",{mode:"date",withTimezone:true}).notNull(),durationMinutes:integer("duration_minutes").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)}),L=pgTable("profil_kat_attendance",{id:text("id").primaryKey().$defaultFn(()=>s()),profilKATId:text("profil_kat_id").notNull().references(()=>_.id),attendanceId:text("attendance_id").notNull().references(()=>E.id),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)}),I=pgTable("user_attendance",{id:text("id").primaryKey().$defaultFn(()=>s()),scheduleId:text("schedule_id").notNull().references(()=>E.id),userId:text("user_id").notNull().references(()=>t.id),status:Ke("status").default("tidak_hadir").notNull(),updatedAt:timestamp("updated_at").$onUpdate(a)});var Qe=pgEnum("itb_guesser_tempat",["Ganesha","Jatinangor","Cirebon"]),y=pgTable("itb_guesser_options",{id:text("id").primaryKey().$defaultFn(()=>s()),tempat:Qe("tempat").notNull(),publicUrl:text("public_url").notNull(),lat:real("lat").notNull(),lng:real("lng").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)}),k=pgTable("itb_guesser_submissions",{id:text("id").primaryKey().$defaultFn(()=>s()),userId:text("user_id").notNull().references(()=>t.id),optionId:text("option_id").notNull().references(()=>y.id),answerLat:real("answer_lat").notNull(),answerLng:real("answer_lng").notNull(),score:real("score").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)}),qi=pgTable("memory_game_scores",{id:text("id").primaryKey().$defaultFn(()=>s()),userId:text("user_id").notNull().unique().references(()=>t.id),score:integer("score").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)});var ie=pgTable("handbook",{mediaId:text("media_id").primaryKey().references(()=>n.id),title:text("title")});var Ui=pgTable("activities",{id:text("id").primaryKey().$defaultFn(()=>s()),day:integer("day").notNull(),title:text("title").notNull(),description:text("description"),startTime:time("start_time").notNull(),endTime:time("end_time").notNull(),location:text("location"),lat:real("lat"),lng:real("lng"),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(a)});var f=pgTable("user_matches",{id:text("id").primaryKey().$defaultFn(createId),topic:text("topic").notNull(),isActive:boolean("is_active").notNull().default(true),isAnonymous:boolean("is_anonymous").notNull().default(true),isRevealed:boolean("is_revealed").notNull().default(false),firstUserId:text("first_user_id").notNull().references(()=>t.id,{onDelete:"cascade"}),secondUserId:text("second_user_id").notNull().references(()=>t.id,{onDelete:"cascade"}),lastMessage:text("last_message"),createdAt:timestamp("created_at").notNull().defaultNow()}),D=pgTable("messages",{id:bigserial("id",{mode:"number"}).primaryKey(),userMatchId:text("user_match_id").notNull().references(()=>f.id,{onDelete:"cascade"}),senderId:text("sender_id").notNull().references(()=>t.id,{onDelete:"cascade"}),content:text("content").notNull(),createdAt:timestamp("created_at",{mode:"date",withTimezone:true}).notNull().defaultNow()});var or=relations(w,({one:e})=>({user:e(t,{fields:[w.id],references:[t.id]})})),nr=relations(t,({one:e,many:r})=>({account:e(w,{fields:[t.id],references:[w.id]}),fotoMedia:e(n,{fields:[t.fotoMediaId],references:[n.id]}),stageProgress:r(b),attendance:r(I),submissions:r(d),createdMedia:r(n),analytics:r(F),classRegistrations:r(z),userMatchesAsFirstUser:r(f,{relationName:"first_user"}),userMatchesAsSecondUser:r(f,{relationName:"second_user"}),messages:r(D,{relationName:"sender"}),itbGuesserSubmissions:r(k)})),_r=relations(B,({one:e})=>({user:e(t,{fields:[B.userId],references:[t.id]})})),lr=relations(Y,({one:e})=>({user:e(t,{fields:[Y.identifier],references:[t.email]})})),ur=relations(n,({one:e,many:r})=>({creator:e(t,{fields:[n.creatorId],references:[t.id]}),handbook:e(ie,{fields:[n.id],references:[ie.mediaId]}),submissions:r(d),itbGuesserOptions:r(y)})),pr=relations(E,({many:e})=>({userAttendance:e(I),profilKATAttendances:e(L)})),fr=relations(I,({one:e})=>({attendance:e(E,{fields:[I.scheduleId],references:[E.id]}),user:e(t,{fields:[I.userId],references:[t.id]})})),gr=relations(L,({one:e})=>({profilKAT:e(_,{fields:[L.profilKATId],references:[_.id]}),attendance:e(E,{fields:[L.attendanceId],references:[E.id]})})),dr=relations(u,({many:e,one:r})=>({userProgress:e(b),questions:e(A),profil:r(_,{fields:[u.profilId],references:[_.id]})})),cr=relations(b,({one:e})=>({user:e(t,{fields:[b.userId],references:[t.id]}),stage:e(u,{fields:[b.stageId],references:[u.id]})})),mr=relations(A,({one:e,many:r})=>({stage:e(u,{fields:[A.stageId],references:[u.id]}),answerOptions:r(G)})),Er=relations(G,({one:e})=>({question:e(A,{fields:[G.questionId],references:[A.id]})})),Nr=relations(_,({many:e,one:r})=>({assignments:e(h),attendances:e(L),stage:r(u,{fields:[_.id],references:[u.profilId]})})),wr=relations(h,({many:e,one:r})=>({submissions:e(d),profil:r(_,{fields:[h.profilKATId],references:[_.id]})})),hr=relations(d,({one:e})=>({assignment:e(h,{fields:[d.assignmentId],references:[h.id]}),user:e(t,{fields:[d.userId],references:[t.id]}),media:e(n,{fields:[d.submissionMediaId],references:[n.id]})})),Ar=relations(M,({one:e,many:r})=>({registrations:r(z)})),Cr=relations(z,({one:e})=>({user:e(t,{fields:[z.userId],references:[t.id]}),class:e(M,{fields:[z.classId],references:[M.id]})})),Sr=relations(F,({one:e})=>({user:e(t,{fields:[F.userId],references:[t.id]})})),qr=relations(f,({many:e,one:r})=>({firstUser:r(t,{fields:[f.firstUserId],references:[t.id],relationName:"first_user"}),secondUser:r(t,{fields:[f.secondUserId],references:[t.id],relationName:"second_user"}),messages:e(D)})),Tr=relations(D,({one:e})=>({senderId:e(t,{fields:[D.senderId],references:[t.id],relationName:"sender"}),userMatch:e(f,{fields:[D.userMatchId],references:[f.id]})})),zr=relations(y,({one:e,many:r})=>({submissions:r(k)})),Or=relations(k,({one:e})=>({user:e(t,{fields:[k.userId],references:[t.id]}),option:e(y,{fields:[k.optionId],references:[y.id]})}));export{w as accounts,or as accountsRelation,qe as accountsRoleEnum,Ui as activities,h as assignmentsProfil,wr as assignmentsProfilRelation,Ke as attendanceStatusEnum,$e as attendanceTypeEnum,E as attendances,pr as attendancesRelation,ke as classEnum,z as classRegistrations,Cr as classRegistrationsRelation,M as classes,Ar as classesRelation,_i as createUserRankingViewIndexes,B as emailVerificationOtps,_r as emailVerificationOtpsRelations,F as endpointAnalytics,Sr as endpointAnalyticsRelation,ie as handbook,y as itbGuesserOptions,zr as itbGuesserOptionsRelations,k as itbGuesserSubmissions,Or as itbGuesserSubmissionsRelations,Qe as itbGuesserTempatEnum,n as media,ur as mediaRelation,qi as memoryGameScores,D as messages,Tr as messagesRelations,L as profilKATAttendances,gr as profilKATAttendancesRelation,_ as profilKATs,Nr as profilKATsRelation,G as questionAnswerOptions,Er as questionAnswerOptionsRelation,Ue as questionTypeEnum,A as questions,mr as questionsRelation,ni as refreshUserRankingView,Me as stageStatusEnum,u as stages,dr as stagesRelation,d as submissionsProfil,hr as submissionsProfilRelation,I as userAttendance,fr as userAttendanceRelation,f as userMatches,qr as userMatchesRelations,oi as userRankingView,b as userStageProgress,cr as userStageProgressRelation,t as users,nr as usersRelation,Y as verificationToken,lr as verificationTokenRelations};