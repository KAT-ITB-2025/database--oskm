...
`;var Ke=pgEnum("attendance_status",["hadir","tidak_hadir"]),$e=pgEnum("attendance_type",["opening","closing"]),E=pgTable("attendances",{id:text("id").primaryKey().$defaultFn(()=>a()),attendanceType:$e("attendance_type").notNull(),dayNumber:integer("day_number").notNull(),startTime:timestamp("start_time",{mode:"date",withTimezone:true}).notNull(),durationMinutes:integer("duration_minutes").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),v=pgTable("profil_kat_attendance",{id:text("id").primaryKey().$defaultFn(()=>a()),profilKATId:text("profil_kat_id").notNull().references(()=>_.id),attendanceId:text("attendance_id").notNull().references(()=>E.id),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),L=pgTable("user_attendance",{id:text("id").primaryKey().$defaultFn(()=>a()),scheduleId:text("schedule_id").notNull().references(()=>E.id),userId:text("user_id").notNull().references(()=>t.id),status:Ke("status").default("tidak_hadir").notNull(),updatedAt:timestamp("updated_at").$onUpdate(r)});var Qe=pgEnum("itb_guesser_tempat",["Ganesha","Jatinangor","Cirebon"]),de=pgTable("itb_guesser_options",{id:text("id").primaryKey().$defaultFn(()=>a()),tempat:Qe("tempat").notNull(),publicUrl:text("public_url").notNull(),lat:real("lat").notNull(),lng:real("lng").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),K=pgTable("itb_guesser_submissions",{id:text("id").primaryKey().$defaultFn(()=>a()),userId:text("user_id").notNull().references(()=>t.id),score:real("score").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)}),Ti=pgTable("memory_game_scores",{id:text("id").primaryKey().$defaultFn(()=>a()),userId:text("user_id").notNull().unique().references(()=>t.id),score:integer("score").notNull(),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)});var te=pgTable("handbook",{mediaId:text("media_id").primaryKey().references(()=>n.id),title:text("title")});var Fi=pgTable("activities",{id:text("id").primaryKey().$defaultFn(()=>a()),day:integer("day").notNull(),title:text("title").notNull(),description:text("description"),startTime:time("start_time").notNull(),endTime:time("end_time").notNull(),location:text("location"),lat:real("lat"),lng:real("lng"),createdAt:timestamp("created_at").defaultNow(),updatedAt:timestamp("updated_at").$onUpdate(r)});var f=pgTable("user_matches",{id:text("id").primaryKey().$defaultFn(createId),topic:text("topic").notNull(),isActive:boolean("is_active").notNull().default(true),isAnonymous:boolean("is_anonymous").notNull().default(true),isRevealed:boolean("is_revealed").notNull().default(false),firstUserId:text("first_user_id").notNull().references(()=>t.id,{onDelete:"cascade"}),secondUserId:text("second_user_id").notNull().references(()=>t.id,{onDelete:"cascade"}),lastMessage:text("last_message"),createdAt:timestamp("created_at").notNull().defaultNow()}),y=pgTable("messages",{id:bigserial("id",{mode:"number"}).primaryKey(),userMatchId:text("user_match_id").notNull().references(()=>f.id,{onDelete:"cascade"}),senderId:text("sender_id").notNull().references(()=>t.id,{onDelete:"cascade"}),content:text("content").notNull(),createdAt:timestamp("created_at",{mode:"date",withTimezone:true}).notNull().defaultNow()});var nr=relations(w,({one:e})=>({user:e(t,{fields:[w.id],references:[t.id]})})),_r=relations(t,({one:e,many:s})=>({account:e(w,{fields:[t.id],references:[w.id]}),fotoMedia:e(n,{fields:[t.fotoMediaId],references:[n.id]}),stageProgress:s(O),attendance:s(L),submissions:s(d),createdMedia:s(n),analytics:s(R),classRegistrations:s(T),userMatchesAsFirstUser:s(f,{relationName:"first_user"}),userMatchesAsSecondUser:s(f,{relationName:"second_user"}),messages:s(y,{relationName:"sender"}),itbGuesserSubmissions:s(K)})),lr=relations(Q,({one:e})=>({user:e(t,{fields:[Q.userId],references:[t.id]})})),ur=relations(V,({one:e})=>({user:e(t,{fields:[V.identifier],references:[t.email]})})),pr=relations(n,({one:e,many:s})=>({creator:e(t,{fields:[n.creatorId],references:[t.id]}),handbook:e(te,{fields:[n.id],references:[te.mediaId]}),submissions:s(d),itbGuesserOptions:s(de)})),fr=relations(E,({many:e})=>({userAttendance:e(L),profilKATAttendances:e(v)})),gr=relations(L,({one:e})=>({attendance:e(E,{fields:[L.scheduleId],references:[E.id]}),user:e(t,{fields:[L.userId],references:[t.id]})})),dr=relations(v,({one:e})=>({profilKAT:e(_,{fields:[v.profilKATId],references:[_.id]}),attendance:e(E,{fields:[v.attendanceId],references:[E.id]})})),cr=relations(u,({many:e,one:s})=>({userProgress:e(O),questions:e(A),profil:s(_,{fields:[u.profilId],references:[_.id]})})),mr=relations(O,({one:e})=>({user:e(t,{fields:[O.userId],references:[t.id]}),stage:e(u,{fields:[O.stageId],references:[u.id]})})),Er=relations(A,({one:e,many:s})=>({stage:e(u,{fields:[A.stageId],references:[u.id]}),answerOptions:s(P)})),Nr=relations(P,({one:e})=>({question:e(A,{fields:[P.questionId],references:[A.id]})})),wr=relations(_,({many:e,one:s})=>({assignments:e(h),attendances:e(v),stage:s(u,{fields:[_.id],references:[u.profilId]})})),hr=relations(h,({many:e,one:s})=>({submissions:e(d),profil:s(_,{fields:[h.profilKATId],references:[_.id]})})),Ar=relations(d,({one:e})=>({assignment:e(h,{fields:[d.assignmentId],references:[h.id]}),user:e(t,{fields:[d.userId],references:[t.id]}),media:e(n,{fields:[d.submissionMediaId],references:[n.id]})})),Cr=relations(D,({one:e,many:s})=>({registrations:s(T)})),Sr=relations(T,({one:e})=>({user:e(t,{fields:[T.userId],references:[t.id]}),class:e(D,{fields:[T.classId],references:[D.id]})})),qr=relations(R,({one:e})=>({user:e(t,{fields:[R.userId],references:[t.id]})})),Tr=relations(f,({many:e,one:s})=>({firstUser:s(t,{fields:[f.firstUserId],references:[t.id],relationName:"first_user"}),secondUser:s(t,{fields:[f.secondUserId],references:[t.id],relationName:"second_user"}),messages:e(y)})),zr=relations(y,({one:e})=>({senderId:e(t,{fields:[y.senderId],references:[t.id],relationName:"sender"}),userMatch:e(f,{fields:[y.userMatchId],references:[f.id]})})),Or=relations(K,({one:e})=>({user:e(t,{fields:[K.userId],references:[t.id]})}));export{w as accounts,nr as accountsRelation,qe as accountsRoleEnum,Fi as activities,h as assignmentsProfil,hr as assignmentsProfilRelation,Ke as attendanceStatusEnum,$e as attendanceTypeEnum,E as attendances,fr as attendancesRelation,ke as classEnum,T as classRegistrations,Sr as classRegistrationsRelation,D as classes,Cr as classesRelation,li as createUserRankingViewIndexes,Q as emailVerificationOtps,lr as emailVerificationOtpsRelations,R as endpointAnalytics,qr as endpointAnalyticsRelation,te as handbook,de as itbGuesserOptions,K as itbGuesserSubmissions,Or as itbGuesserSubmissionsRelations,Qe as itbGuesserTempatEnum,n as media,pr as mediaRelation,Ti as memoryGameScores,y as messages,zr as messagesRelations,v as profilKATAttendances,dr as profilKATAttendancesRelation,_ as profilKATs,wr as profilKATsRelation,P as questionAnswerOptions,Nr as questionAnswerOptionsRelation,Ue as questionTypeEnum,A as questions,Er as questionsRelation,_i as refreshUserRankingView,Me as stageStatusEnum,u as stages,cr as stagesRelation,d as submissionsProfil,Ar as submissionsProfilRelation,L as userAttendance,gr as userAttendanceRelation,f as userMatches,Tr as userMatchesRelations,ni as userRankingView,O as userStageProgress,mr as userStageProgressRelation,t as users,_r as usersRelation,V as verificationToken,ur as verificationTokenRelations};